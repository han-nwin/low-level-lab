...
By the end of the lab, this call must configure the default RP2350 system clock:

```rust
let clocks = unsafe { clock::init_150mhz() };
assert_eq!(clocks.sys_hz, 150_000_000);
```
...
After this lab, “the CPU runs at 150 MHz” should no longer feel like a magical
...
1. Call `init_150mhz()` before enabling interrupts or starting core 1.
...
6. During the first implementation, use only the known-good 150 MHz settings.
...
POSTDIV1   = 5
POSTDIV2   = 2
PLL output = 1500 MHz / 5 / 2 = 150 MHz
```
...
4. Which value is the internal VCO frequency: 150 MHz or 1500 MHz?
...
```rust
unsafe fn configure_pll_sys_150mhz();
```
...
6. Write POSTDIV1 = 5 and POSTDIV2 = 2.
...
pub unsafe fn init_150mhz() -> Clocks;
```
...
Stretch goal: route `CLOCK GPOUT0` to GPIO21 and divide `clk_sys` by 150.
...
7. Configure the RP2350 frequency counter and measure `clk_sys` internally.
...
3. Why is the CPU clock 150 MHz when the VCO is 1500 MHz?
...
8. What extra issues appear when overclocking beyond the rated 150 MHz?
