...
fn main() -> ! {
    ...
    unsafe {
        ...
        // 0x08 sets P3, commonly the LCD backlight.
        for _ in 0..3 {
            i2c_write(0x00); // P3 = 0 backlight off
            cortex_m::asm::delay(3_000_000); // delay a bit
            i2c_write(0x08); // P3 = 1 backlight on
            cortex_m::asm::delay(3_000_000); // delay a bit
        }
        ...
        // Embedded main must never return.
        loop {
            // NOTE:
    ...
    unsafe {
        ...
        loop {
            ...
            if status & (1 << 6) != 0 {
                ...
                logln!("TX_ABRT: reason = 0x{:x}", reason);
...
unsafe fn lcd_write_nibble(nibble: u8, is_data: bool) {
    unsafe {
...
unsafe fn startup_sequence() {
    unsafe {
        ...
        lcd_write_nibble(0x02, false); // Enter 4-bit mode

        // From here, commands are complete 8-bit values sent as two nibbles.
        lcd_write_byte(0x28, false); // 4-bit mode, two lines, 5×8 characters
        lcd_write_byte(0x08, false); // Display off
        lcd_write_byte(0x01, false); // Clear display
        cortex_m::asm::delay(300_000); // Clear requires about 2 ms
        lcd_write_byte(0x06, false); // Move cursor right after each character
